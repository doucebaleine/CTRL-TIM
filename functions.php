...
add_action('template_redirect', function() {
    if (isset($_GET['projet_id'])) {
        ...
        if (function_exists('ctrltim_get_projet_by_id')) {
            ...
            if ($projet) {
                // Utiliser le template `template-projet.php` pour l'affichage complet
                // Le template attend la variable `project_id` (anglais), on la renseigne ici
                $_GET['project_id'] = intval($projet_id);
                // Localiser et inclure le template dans le thème
                $tpl = locate_template('template-projet.php');
                if ($tpl) {
                    include $tpl;
                    exit;
                }
